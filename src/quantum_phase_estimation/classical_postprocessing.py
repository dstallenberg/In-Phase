...
def binary_fraction_to_decimal(bin_frac, desired_accuracy, nancillas):
	"""Returns the decimal fraction of a binary fraction formatted as a binary 
	number."""
	frac = 0
	bin_string = str(bin(int(bin_frac)))[-nancillas::]
	print(bin_string)
...
def print_result(bin_frac, desired_accuracy, nancillas):
	"""Prints the phase shift in radians."""
	print("Phase shift is {0:0.{accuracy}f}+/-{1:0.{accuracy}f} radians".format(
			binary_fraction_to_decimal(bin_frac, desired_accuracy, nancillas)*2*np.pi,
...
if __name__ == '__main__':
	...
	print(res)
	desired_accuracy = 3
	print(binary_fraction_to_decimal(res, desired_accuracy, nancillas))
	print_result(res, desired_accuracy, nancillas)